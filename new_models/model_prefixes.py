...
def get_gpt2_prefixes(sentence, tokenizer):
    ...
    for length in range(1, num_tokens):
...
def get_bertlike_mask_func(tokenizer):
    ...
    def _bertlike_masks_func(sentence, tokenizer):
        
        sentence_tokens = tokenizer.encode(sentence)
        num_tokens = len(sentence_tokens)
        tokens = []

        for masked_idx in range(1, num_tokens):
