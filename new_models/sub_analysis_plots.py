...
def to_negative_surprisal(prob_series):
    return np.log10(prob_series)

# 6/1 : https://seaborn.pydata.org/generated/seaborn.violinplot.html

def prep_single_model_df_violin(model_name, df, is_neg_surprisal = False):
    ...
    orig_probs = to_negative_surprisal(df['orig_prob']) if is_neg_surprisal else df['orig_prob']
    edited_probs = to_negative_surprisal(df['edited_prob']) if is_neg_surprisal else df['edited_prob']
...
def gen_violin_plots(all_changes_probs, use_neg_surprisal):
    """
    ...
    return pd.concat([
        prep_single_model_df_violin(model_name, filter_nan_df(raw_df), 
                                   is_neg_surprisal = use_neg_surprisal)
