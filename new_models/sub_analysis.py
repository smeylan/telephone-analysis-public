...
def process_substitution_entry(df_entry, model, tokenizer, prefix_func, verifying = False):
    ...
    orig_prob = process_single_substitution(df_entry['sentence'], df_entry['sWord'], df_entry['sCounter'], verifying = verifying, *model_args)
    edited_prob = process_single_substitution(df_entry['response'], df_entry['rWord'],  df_entry['rCounter'], verifying = verifying, *model_args)
    
    # result per item in the tuple is word_prob, all_probs if verifying, else, just the word_prob
